    ...
    <body>
        ...
        <div class="content">
            ...
            <h1 class="name">${company.businessName ? company.businessName : company.name}</h1>
              ...
              <hr>
              <div>
                  <p style="font-size: 18px">${Number(balanceDue) <= 0 ? 'Thank you for paying your bill' : type} ${Number(balanceDue) != 0 ? 'for' : ''} <span style="font-weight: 700">${Number(balanceDue) <= 0 ? '' : `₱${balanceDue}`}</span> ${Number(balanceDue) <= 0 ? '' : `due by`} <span style="font-weight: 700">${Number(balanceDue) <= 0 ? '' : moment(dueDate).format("MMM Do YYYY")}</span></p>
                  <p style="font-size: 18px">${Number(balanceDue) <= 0 ? '' : 'Please pay your bill on or before due date to avoid disconnection. Thank you'}</span></p>
              </div>
